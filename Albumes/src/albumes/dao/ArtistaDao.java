    ...
    public void actualizarArtista(Artista artista) {
        for (int i = 0; i < artistas.size(); i++) {
            if (artistas.get(i).getId() == artista.getId()) {
                ...
                artistas.get(i).setApellidoMaterno(artista.getApellidoMaterno());
                artistas.get(i).setApellidoPaterno(artista.getApellidoPaterno());
                artistas.get(i).setGenero(artista.getGenero());
